...
Write-Host "Building and injecting Rust module (ascii-render-native)..." -ForegroundColor Yellow
if (!(Get-Command cargo -ErrorAction SilentlyContinue)) {
    Write-Host "Warning: Rust (cargo) not found. ASCII rendering might not work." -ForegroundColor Red
    ...
    Push-Location "$ProjectRoot\telegram_textual_tui\ascii_tool"
    pipx runpip telegram-textual-tui install .
    Pop-Location
}

Write-Host "---------------------------------------" -ForegroundColor Green
Write-Host "Success! TGT is now installed with ASCII support." -ForegroundColor Green
